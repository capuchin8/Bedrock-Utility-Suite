 ...
 */
import { system, world, Player, EquipmentSlot, CommandPermissionLevel, CustomCommandStatus, CustomCommandParamType, } from "@minecraft/server";
...
function hideInventory(player) {
    ...
    if (!(playerInvComponent === null || playerInvComponent === void 0 ? void 0 : playerInvComponent.container) || !equippable)
        return;
    ...
    if (!(storageInvComponent === null || storageInvComponent === void 0 ? void 0 : storageInvComponent.container)) {
        storage.remove();
    ...
    for (let i = 0; i < playerContainer.size; i++) {
        ...
        if (item)
            storageContainer.setItem(i, item);
    }
    // Move armor + offhand into the storage entity (36-40).
    for (const slot of ARMOR_SLOTS) {
        const item = equippable.getEquipment(slot);
        if (item)
            storageContainer.setItem(ARMOR_STORAGE_SLOT[slot], item);
...
function restoreInventory(player) {
    ...
    if (!storage || !storage.isValid) {
        player.sendMessage("§cCouldn't find your stored inventory. It may be in an unloaded area — try again after moving around a bit.");
        return;
    }
    const storageInvComponent = storage.getComponent("minecraft:inventory");
    const playerInvComponent = player.getComponent("minecraft:inventory");
    const equippable = player.getComponent("minecraft:equippable");
    if (!(storageInvComponent === null || storageInvComponent === void 0 ? void 0 : storageInvComponent.container) || !(playerInvComponent === null || playerInvComponent === void 0 ? void 0 : playerInvComponent.container) || !equippable)
        return;
    const storageContainer = storageInvComponent.container;
    const playerContainer = playerInvComponent.container;
    playerContainer.clearAll();
    ...
    if (selectedSlot !== undefined)
        player.selectedSlotIndex = selectedSlot;
...
function seeInventory(runner, target) {
    ...
    if (invComponent === null || invComponent === void 0 ? void 0 : invComponent.container) {
        const container = invComponent.container;
        for (let i = 0; i < container.size; i++) {
            const item = container.getItem(i);
            if (item)
                runner.sendMessage(formatItemLine(`Slot ${i}`, item));
        }
    }
    if (equippable) {
        for (const slot of ARMOR_SLOTS) {
            const item = equippable.getEquipment(slot);
            if (item)
                runner.sendMessage(formatItemLine(slot, item));
        }
    }
...
function seeEnderChest(runner, target) {
    ...
    if (!(enderInv === null || enderInv === void 0 ? void 0 : enderInv.container)) {
...
system.beforeEvents.startup.subscribe((init) => {
    ...
    registry.registerCommand({
        ...
        system.run(() => {
            const players = (targetEntities !== null && targetEntities !== void 0 ? targetEntities : []).filter((e) => e instanceof Player);
            if (players.length === 0) {
            ...
            for (const target of players) {
                switch (action) {
                    ...
                    case "see":
                        if (runner)
                            seeInventory(runner, target);
                        break;
                    case "see_echest":
                        if (runner)
                            seeEnderChest(runner, target);
                        break;
                }
            }
        });
